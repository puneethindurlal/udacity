...
def load_data(database_filepath):
    ''' loads data from sql database and
     returns feature dataframe, label-data DataFrame, labels as list'''
    # load data from database
    engine = create_engine('sqlite:///'+database_filepath)
    df = pd.read_sql('SELECT * FROM messages_categories', con=engine)
    X = df.message.values 
...
def build_model():
    '''builds pipeline, sets parameter, does a Gridsearch, and returns a model'''
    pipeline = Pipeline([
...
def evaluate_model(model, X_test, Y_test, category_names):
    '''evaluates the model using sklearn classification_report'''
